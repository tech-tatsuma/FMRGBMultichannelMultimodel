...
class MultiTaskMultiChannelModel(nn.Module):
    def __init__(self, channels, depth, height, width, hidden_dim, num_experts=3):
        ...
        self.gate_output_layer = nn.Linear(hidden_dim, num_experts)
    ...
    def forward(self, x):
        ...
        gate_logits = self.gate_output_layer(last_combined_out)
...
class MoESeq2Seq(nn.Module):
    def __init__(self, num_experts, channels, depth, height, width, hidden_dim):
        super(MoESeq2Seq, self).__init__()

        # ゲートネットワーク
        self.gate = MultiTaskMultiChannelModel(channels, depth, height, width, hidden_dim)

        # エキスパートネットワーク（３つ）
        self.experts = nn.ModuleList([Seq2Seq(channels, depth, height, width, hidden_dim, num_tasks=5) for _ in range(num_experts)])

    def forward(self, x):
        # ゲートネットワークを通過させて重みを取得
        gate_output = self.gate(x)

        # 各エキスパートネットワークの出力を取得
        expert_outputs = [expert(x) for expert in self.experts]

        # ゲートの出力を使って各エキスパートの出力を重み付け
        final_output = 0
